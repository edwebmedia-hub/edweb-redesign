...
def li(items):
    out = []
    for it in items:
        out.append(f'            <li><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M20 6L9 17l-5-5"/></svg>{it}</li>')
    return '\n'.join(out)

def gallery_card(img, alt, small, name):
    return f'''          <a href="#" class="gallery-card" onclick="return false;">
            <img src="../assets/destinations/{{slug}}/{img}" alt="{alt}" loading="lazy" />
            <span class="gallery-caption"><small>{small}</small>{name}</span>
          </a>'''
...
def build(d):
    ...
    tagline = d['tagline']
    overview = d['overview']
    highlights = li(d['highlights'])
    culture_heading = d.get('culture_heading', 'Culture')
    culture = d['culture']
    tips_heading = d.get('tips_heading', 'Travel Tips')
    tips = li(d['tips'])
    best_time = d['best_time']
    gem_location = d['gem_location']
    gem_title = d['gem_title']
    gem_meta1 = d['gem_meta1']
    gem_meta2 = d['gem_meta2']
    gem_desc = d['gem_desc']
    gallery = '\n'.join(
        gallery_card(g['img'], g['alt'], g['small'], g['name']).replace('{slug}', slug)
        for g in d['gallery']
    )
...
<head>
  ...
  <title>{name} | Navigator Vietnam</title>
...
<body>
  ...
  <div class="mobile-menu" id="mobileMenu">
    ...
    <a href="../contact.html" class="btn btn-accent">Plan Your Trip</a>
  </div>
  ...
  <main id="main">
    ...
    <section class="page-hero dest-hero has-photo">
      <img src="../assets/destinations/{slug}/{slug}-hero.jpg" alt="{d['hero_alt']}" class="dest-hero-bg" fetchpriority="high" />
      <div class="dest-hero-overlay"></div>
      <div class="container">
        <span class="breadcrumb reveal"><a href="../index.html">Home</a> / Destination Details</span>
        <h1 class="reveal">{name}</h1>
        <div class="dest-hero-divider reveal"></div>
        <p class="page-hero-lead dest-hero-lead reveal">{tagline}</p>
      </div>
    </section>
...
{tips}
          ...
          <a href="../contact.html?destination={d['contact_dest']}" class="btn btn-primary dest-content-cta">
            Plan Your Trip
            <svg class="btn-arrow" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M5 12h14M13 6l6 6-6 6"/></svg>
          </a>
        </div>
        ...
        <aside class="dest-sidebar reveal">
          <article class="adv-card">
            <div class="adv-card-media">
              <img src="../assets/destinations/{slug}/{slug}-hidden-gem.jpg" alt="{d['gem_img_alt']}" loading="lazy" />
            </div>
            <div class="adv-card-body">
              <span class="adv-card-location">{gem_location}</span>
              <div class="adv-card-head">
                <h3>{gem_title}</h3>
              </div>
              <div class="adv-card-meta">
                <span><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="12" cy="12" r="9"/><path d="M12 7v5l3 2"/></svg>{gem_meta1}</span>
                <span class="adv-card-diff">{gem_meta2}</span>
              </div>
              <p>{gem_desc}</p>
            </div>
          </article>
    ...
    <div class="container">
      <div class="footer-grid">
        ...
        <div class="footer-col footer-contact">
          <h4>Get in Touch</h4>
          <ul>
            <li><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8"><path d="M20 10c0 6-8 12-8 12s-8-6-8-12a8 8 0 1116 0z"/><circle cx="12" cy="10" r="3"/></svg><span>La Grace 5, 34 Ngõ 20 P. Quảng Khánh, Hà Nội</span></li>
      ...
      <div class="footer-bottom">
        <span>© 2026 Navigator Vietnam. All Rights Reserved.</span>
